...
if ($_SESSION["nombre"]==""){
 ...
 header("Location: index4.html");
  ...
  //no hago nada solo continuo en la pagina
    ...
    //para editarlas primeros las elimino y luego vulevo a ingresarlas
     $sql7="delete from gradomateria where id_grado='$grado' and id_year='$year'";
     $delete = pg_query($conn,$sql7); 

    foreach ($_POST["cod"]  as $mates) {


       $mate = pg_escape_string($conn, $_POST["cod"][$mates]);

       $sql2="select * from gradomateria where id_grado='$grado' and id_materia='$mate' and id_year='$year'"; 
       $result2= pg_query($conn,$sql2);
       $filas=pg_num_rows($result2); // en este caso 3

      if($filas==0){

        $sql="insert into gradomateria ( id_grado, id_year, id_materia ) values ( '$grado','$year', '$mate')"; 
        $actualizar2= pg_query($conn,$sql);
...
<header class="cabecera">
 ...
 echo " <hgroup class='titulos-wrap'>
    <h3 class='titulo wow bounceIn' data-wow-delay='0.3s'>Carga académica Ingresada correctamente</h3>
    <h3 class='subtitulo wow bounceIn' data-wow-delay='0.5s'>Ahora puedes editar la carga académica de otro grado.</h3>
    </hgroup>";
          }
  ...
  echo "<hgroup class='titulos-wrap'>
    <h3 class='subtitulo wow fadeIn' data-wow-delay='0.6s' align='justify'>No se pudo ingresar la carga académica correctamente, es probable que ya se halla ingresado previamente, por favor revisa bien y vuelve a intentarlo.</h3>
    </hgroup>";
...
<a href="editar_carga1.php"><button class="btn btn-primary">Regresar</button></a>  
                    </div>
                    <!-- /.section title start-->
